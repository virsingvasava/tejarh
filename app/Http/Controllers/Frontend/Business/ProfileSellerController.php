...
class ProfileSellerController extends Controller
{
    ...
    public function index($id)
    {
        ...
        $userStory = UserStory::select('category_id')->with('category')->where('user_id', '=', $userId)->where('is_paid','=','1')->where('deleted_at', '=', NULL)->groupBy('category_id')->get();
        $storyAll = UserStory::select('category_id')->where('deleted_at', '=', NULL)->where('user_id', '=', $userId)->where('is_paid','=','1')->groupBy('category_id')->get();
        ...
        $userStoryNormal = UserStory::select('category_id')->with('category')->where('is_paid','=','1')->where('user_id', '=', $userId)->where('deleted_at', '=', NULL)->groupBy('category_id')->get();
        $storyAllNormal = UserStory::select('category_id')->where('deleted_at', '=', NULL)->where('is_paid','=','1')->where('user_id', '=', $userId)->groupBy('category_id')->get();
        ...
        foreach ($CheckCategoryNormal as $key => $value) {
            $storyAllNormal = UserStory::with('user', 'category')->where('user_id', $userId)->where('is_paid','=','1')->where('deleted_at', '=', NULL)->where('category_id', $value)->get()->toArray();
        ...
        return view('frontend.business.pages.profiles.seller_profile', compact(
            'pages',
            'userBannerReplace',
            'bannerReplace',
            'category',
    ...
    public function followers22(Request $request)
    {

        $checkFollow = UserFollowers::where(['following_id' => $request->following_id])->first();
        //p($checkFollow);
        if (!empty($checkFollow)) {
            $status = $checkFollow->follow_unfollow_status;
            if ($status == 1) {
                $status_update = UserFollowers::where(['following_id' => $request->following_id])->first();
                $status_update->follow_unfollow_status = false;
                $status_update->save();
                return true;

                // return response()->json(['status' => 200, 'success' => Lang::get('business_messages.follow.success.un_follow_successfully_done')], 200);
            }
            if ($status == 0) {
                $status_update = UserFollowers::where(['following_id' => $request->following_id])->first();
                ...
                // return response()->json(['status' => 200, 'success' => Lang::get('business_messages.follow.success.follow_successfully_done')], 200);
            }
        } else {
    ...
    public function loadFollower(Request $request)
    {
        $userId = $request->productAuthorId;
        $responseData = UserFollowers::where(['following_id' => $userId, 'follow_unfollow_status' => '1'])->sum('id');
        return $responseData;
    }

    public function loadFollowing(Request $request)
    {
        $userId = $request->productAuthorId;
        $responseData = UserFollowers::where(['following_id' => $userId, 'follow_unfollow_status' => '1'])->sum('id');
        return $responseData;
