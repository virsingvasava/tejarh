...
class MyOrdersController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($orders as $orderKey => $orderValue)
        {
            ...
            if($orderValue['return_order_id'] == NULL)
            {
                ...
                $itemArray[$orderKey]['getAddressId'] = $getAddressId; 
            ...
            {
                $orderItems = OrderItem::where('id',$orderValue['return_order_id'])->first();
                $orderItemId = $orderItems->id;
                $getAddressReturnId = OrderDeliveryAddress::where('order_item_id', $orderItemId)->first();
        ...
        return view('frontend.business.pages.orders.index', compact('itemArray', 'orders_list', 'reviewRatingArray','randomNumber'));
    }
    ...
    public function filter_ajax(Request $request)
    {
        if ($request->ajax() && isset($request->order_status)) {
            ...
            $randomNumber = random_int(100000, 999999);
            return view('frontend.business.pages.orders.filter_order', compact('orders','randomNumber'));
        } else {
            $user = Auth::user();
    ...
    public function orderFilter(Request $request){
        ...
        $randomNumber = random_int(100000, 999999);
        return view('frontend.business.pages.orders.order_filter', compact('arrayFilter','randomNumber'));
    ...
    public function busines_review_post_user_details(Request $request){
        $user = User::where('id', $request->userId)->first();
        return response()->json(['userData' => $user]);
    }
    public function busines_review_post_store(Request $request){

        $new_user = new ReviewRatings;
        $image = $request->business_review_items_Image;
        if ($request->has('business_review_items_Image')) {
            $imagename = $request->business_review_items_Image;
    ...
    public function busines_retrive_reviews(Request $request){

        $totalReviewAvg = ReviewRatings::Where('item_id', $request->itemId)->pluck('rating_star')->avg();
        ...
        return view('frontend.business.pages.orders.review_list', compact('reviewRatingArray', 'totalReviewAvg', 'totalReviewCount'));
    }
    # Reviews Module End#\
    ...
    public function track_orders($id)
    {
        ...
        return view('frontend.business.pages.orders.order_track',compact('getAddress'));
    }
