...
class UnusedItemsController extends Controller
{
    public function index() 
    {   
      
        $unusedItemsList = Item::where(['condition_id' => '3', 'deleted_at' => NULL])
        ->where('status','=','1')
        ...
        {
            $unusedItemsArray[$key] = $value;
            $itemsImage = ItemsImages::where('item_id',$value['id'])->first();
            $unusedItemsArray[$key]['item_pictures'] = $itemsImage;

            $condition = Condition::where('id',$value['condition_id'])->first();
            $unusedItemsArray[$key]['condition'] = $condition;

            $brand = Brand::where('id',$value['brand_id'])->first();
            $unusedItemsArray[$key]['brand'] = $brand;

            $store = Store::where('id',$value['store_id'])->first();
            $unusedItemsArray[$key]['store'] = $store;

            $boostItem = BoostItem::where('item_id',$value['id'])->where('is_paid','1')->first();
            $unusedItemsArray[$key]['boostItem'] = $boostItem;

            $user = User::where('id', $value['user_id'])->first();
            $userCity = $user->city_id;
            $getCity = City::where('id', $userCity)->first();
            $unusedItemsArray[$key]['city'] = $getCity;
        }
        return view('frontend.business.pages.items.unused_Items',compact('unusedItemsArray'));   
    }
    ...
    public function myproduct_details($id)
    {
        ...
        return view('frontend.business.pages.items.myproduct_details', compact('stockArray','itemArray','boostItemArray','relatedItemArray','choice_optionsarray'));
    }
}
