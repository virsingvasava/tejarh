...
use App\Models\MyFatoorahInvoice;
...
class OrdersDetailsController extends Controller
{
    public function index()
    {
        ...
        $checkoutUrl = CheckOutUserDetails::where('user_id', Auth::id())->first();
        $checkoutData = [];

        if (!empty($checkoutUrl['item_id'])) {

            $checkoutData['invoiceStatus'] = CheckOutUserDetails::where(['item_id' => $checkoutUrl['item_id'], 'user_id' => $checkoutUrl['user_id']])->latest('id')->first();
        }
        $arrayData = [];

        if (!empty($checkoutUrl['item_id'])) {

            $arrayData['payable_datails']  = Item::where(['id' => $checkoutUrl['item_id'], 'user_id' => $checkoutUrl['user_id']])->where('status', '=', '0')->where('status', '=', '1')->latest('id')->withTrashed()->first();
        }
        return view('frontend.users.my_orders.order_summary_payment', compact('checkoutUrl', 'arrayData', 'checkoutData'));
    }
    ...
    public function checkout(Request $request, $id)
    {
        ...
        if (!empty($checkCart) && count($checkCart) > 0) {
            ...
            if (!empty($checkOutUserId)) {
                ...
                return view('frontend.users.checkout.index', compact('checkCart', 'itemArray', 'json', 'monthArray', 'grand_total', 'userAddressDetail', 'checkOutUserId', 'sumPrice'));
            }
        } else {
            return redirect()->route('frontend.users.order-details.checkout_empty');
        }
    }

    public function checkout_empty()
    {
        return view('frontend.users.checkout.index');
    }
    ...
    public function card_details($id)
    {
        ...
        // return view('frontend.users.my_orders.card_details');

        return view('frontend.users.my_orders.sample', compact('items', 'cart_price'));
    }

    public function thankyou(Request $request, $token)
    {
        return view('frontend.users.my_orders.thankyou');
    }
    ...
    public function order_placed(Request $request)
    {
        ...
        $save_token->user_id  = $authUser->id;
        ...
        try {
            ...
            if (!empty($getCart) && count($getCart) > 0) {
                foreach ($getCart as $k => $v) {
                    ...
                    $postDataItem = [];
                    $fetchItem = Item::whereIn('id', $itemArr)->withTrashed()->get();
            ...
            return view('frontend.users.my_orders.order_placed_successful')->with('success', __('Payment SuccessFull'));
    ...
    public function order_details($id)
    {
        ...
        if ($orderDetails['return_order_id'] == NULL) {
            ...
            return view('frontend.users.my_orders.order_details', compact(
                'orderDetails',
                'orderAddress',
                'reviewRatingArray',
                'orderGetUser',
    ...
    public function pdf_download_invoice($id)
    {
        ...
        return Pdf::loadView('frontend.users.my_orders.download_invoice', [
            'order' => $order,
            'itemArray' => $itemArray,
            'itemUser' => $itemUser,
    ...
    public function qty_add_minus(Request $request)
    {
        ...
        $total_amount = numberFormat($session_price * $session_quantity);
    ...
    public function removedItems(Request $request)
    {
    ...
    public function addToCart(Request $request)
    {
        ...
        $update->attributevariant = $request->attribute_id;
        $update->save();
    ...
    public function removedItem($id)
    {
        ...
        if (!empty($AuthUserId)) {
            ...
            return redirect('frontend.users.order-details.checkout_empty');
        }
    }
    ...
    public function orderPaymentChoose($id)
    {
        ...
        if (!empty($checkPaymentModeCheck)) {
            ...
            return view('frontend.users.checkout.order_payment', compact('total_shipping', 'itemsArray', 'walletTotal', 'checkPaymentMode', 'total_amount', 'userAddressDetail', 'userCartDetail', 'monthArray', 'itemsList'));
        }

        return view('frontend.users.checkout.order_payment', compact('total_shipping', 'itemsArray', 'walletTotal', 'total_amount', 'userAddressDetail', 'userCartDetail', 'monthArray', 'itemsList'));
    }
    else{
        return redirect()->route('frontend.users.site.index');
    }
    ...
    public function orderPaymentCOD()
    {
        ...
        PaymentSelectOption::where('user_id', $authUser->id)->delete();
        cardDetails::where('user_id', $authUser->id)->delete();
        return view('frontend.users.my_orders.order_placed_successful')->with('success', __('Order Placed SuccessFull'));
    }
