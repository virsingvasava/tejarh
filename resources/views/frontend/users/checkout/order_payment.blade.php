            ...
            <div class="col-md-3">
                <div class="right-order-summary-payment">
                    ...
                    <div class="form-group submit">
                        ...
                        <a href="{{ route('frontend.users.order-details.orderPaymentCOD') }}" class="btn btn-primary">Placed Order</a>
                        @elseif($checkPaymentMode['payment_mode'] === 'apple_pay')
                        <input type="submit" class="btn btn-primary" value="Pay Now">
                        @elseif($checkPaymentMode['payment_mode'] === 'wallet')
                        @if($walletTotal < $roundAmount) <input type="submit" class="btn btn-primary" value="Insuffient Amount">
                        @else
                        <a href="{{ route('frontend.users.order-details.order_wallet') }}" class="btn btn-primary">Pay Now</a>
                        @endif
                        @endif
                        @endif
                    </div>
...
<div class="modal fade" id="add-card-payment" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('frontend.users.order-details.order_placed') }}" method="POST" id="usersPaymentCardForm">
                @csrf
    ...
    $(document).ready(function() {
        $('.cardSelect').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '{{ route("frontend.users.order-details.orderPaymentSelect") }}',
                data: {
        ...
        return new Promise(function(resolve, reject) {
            ...
            processPayment(paymentData)
                .then(function() {
                    ...
                    window.location.href = "{{ url('/order-details/order-successfull') }}";
                })
                .catch(function() {
                    resolve({
    ...
    function processPayment(paymentData) {
        return new Promise(function(resolve, reject) {
            setTimeout(function() {
                ...
                var token = "{{ csrf_token() }}";
                var total_amount = <?php echo (json_encode((string)$roundAmount)); ?>;
                $.ajax({
                    url: "{{ route('frontend.users.order-details.orderPaymentgpay') }}",
                    type: "POST",
                    dataType: "json",
                    data: {
