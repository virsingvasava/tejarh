...
@section('content')
            ...
            <div class="content-header row">
                <div class="content-header-left col-12 mb-2 mt-1">
                    <div class="breadcrumbs-top">
                        <h5 class="content-header-title float-left pr-1 mb-0">@lang('messages.business_user.business_users')</h5>
                        <div class="breadcrumb-wrapper d-none d-sm-block">
                            <ol class="breadcrumb p-0 mb-0 pl-1">
                                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="bx bx-home-alt"></i></a>
                                </li>
                                <li class="breadcrumb-item active">@lang('messages.business_user.business_users_list')
                                </li>
                            </ol>
                        </div>
                    </div>
                                ...
                                <div class="table-responsive">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('messages.business_user.image')</th>
                                            <th>@lang('messages.business_user.name')</th>
                                            <th>@lang('messages.business_user.email')</th>
                                            <th>@lang('messages.business_user.last_activity')</th>
                                    ...
                                    <tbody>
                                        @if (!empty($users) && count($users) > 0)
                                            @foreach ($users as $key => $value)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="checkbox" data-id="{{ $value->id }}" name="status"
                                                            class="js-switch" {{ $value->status == 1 ? 'checked' : '' }}>
                                                    </td>
                                                    <td>
                                                        <div class="dropdown">
                                                            <span
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.business_users.view', base64_encode($value->id)) }}"><i
                                                                        class="bx bx-show-alt mr-1"></i>
                                                                    @lang('messages.common.view')</a>
                                                                    <a class="dropdown-item"
                                                                    href="{{ route('admin.business_users.edit', base64_encode($value->id)) }}"><i
                                                                        class="bx bx-edit-alt mr-1"></i>
                                                                    @lang('messages.common.edit')</a>
                                                                <a href="javascript:void(0)"
                                                                    class="dropdown-item tejarh_delete_button"
                                                                    data-toggle="modal" data-target="#tejarhDeleteModel"
                                                                    data-id="{{ $value->id }}"><i
                                                                        class="bx bx-trash mr-1"></i>@lang('messages.common.delete')</a>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
                                        ...
                                        @else
                                            <tr>
                                                <td colspan="10">@lang('messages.business_user.not_found_business_user')</td>
                                            </tr>
                                        @endif
                                    </tbody>
                                    </table>
    ...
    <div class="modal fade" id="tejarhDeleteModel" tabindex="-1" role="dialog" aria-labelledby="tejarhModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="mb-0">
                        <strong>@lang('messages.business_user.are_you_sure_to_delete_business_user')</strong>
                    </p>
                </div>
                <form action="{{ route('admin.business_users.destroy') }}" method="POST">
                    @csrf
                    <input type="hidden" name="business_user_id" class="business_user_id">
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-light-primary ml-1"> <i
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('.js-switch').change(function() {
                ...
                let business_user_id = $(this).data('id');
                var token = "{{ csrf_token() }}";
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: '{{ route("admin.business_users.business_user_status_update") }}',
                    data: {
                        'status': status,
        ...
        $(document).on('click', '.tejarh_delete_button', function() {
            ...
            $('.business_user_id').val($(this).attr('data-id'));
        })
