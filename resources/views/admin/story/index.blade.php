      ...
      <div class="content-header row">
         <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
               <h5 class="content-header-title float-left pr-1 mb-0">@lang('messages.story.stories')</h5>
               <div class="breadcrumb-wrapper d-none d-sm-block">
                  <ol class="breadcrumb p-0 mb-0 pl-1">
                     <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="bx bx-home-alt"></i></a>
                     </li>
                     <li class="breadcrumb-item active">@lang('messages.story.story_list')
                     </li>
                  </ol>
               </div>
            </div>
               ...
               <div class="col-12">
                  <div class="card">
                     <div class="card-header">
                        <h4 class="card-title">@lang('messages.story.story_list')</h4>
                     </div>
                     <div class="card-body card-dashboard">
                        <div class="table-responsive">
                            @if (App::isLocale('en'))
                               <table class="table zero-configuration" id="story_english_table">
                            @else
                               <table class="table zero-configuration" id="story_arabic_table">
                            @endif
                              <thead>
                                 <tr>
                                    <th>@lang('messages.common.sr_no')</th>
                                    <th>@lang('messages.story.image_video')</th>
                                    <th>@lang('messages.story.user_name')</th>
                                    <th>@lang('messages.story.product_name')</th>
                                    <th>@lang('messages.story.price')</th>
                                    <th>@lang('messages.common.status')</th>
                                    <th>@lang('messages.common.action')</th>
                                 </tr>
                              </thead>
                              @if(!empty($story) && count($story) > 0)
                              @foreach($story as $key => $value)
                              <tr>
                                 <td>#{{$key+1}}</td>
                                 <td class="text-center">
                                    @if($value->video_or_image_file != '' && file_exists(public_path('assets/stories/'.$value->video_or_image_file)))
                                    <img src="{{asset('assets/stories/'.$value->video_or_image_file)}}" style="height: 70px; width: 70px;" alt="Picture" class="img-profile rounded-circle" />
                                    @else
                                    <img src="{{asset('assets/stories/default.png')}}" alt="Picture" style="height: 70px; width: auto;" class="img-profile rounded-circle" />
                                    @endif
                                 </td>
                                 ...
                                 <td>
                                    <div class="dropdown">
                                       ...
                                       <div class="dropdown-menu dropdown-menu-right">
                                          ...
                                          <a class="dropdown-item" href="{{route('admin.story.view',base64_encode($value->id))}}"><i class="bx bx-show-alt mr-1"></i> @lang('messages.common.view')</a>

                                          <!-- <a class="dropdown-item" href="javascript:void(0)"><i class="bx bx-edit-alt mr-1"></i> @lang('messages.common.edit')</a> -->
                              ...
                              @else
                              <tr>
                                 <td colspan="10">@lang('messages.story.not_found_story')</td>
                              </tr>
                              @endif
                           </table>
                        </div>
   ...
   <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
         ...
         <div class="modal-body">
            <p class="mb-0">
               <strong>@lang('messages.story.are_you_sure_delete_story')</strong>
            </p>
         </div>
         <form action="{{route('admin.story.destroy')}}" method="POST">
            @csrf
            <input type="hidden"  name="story_id" class="story_id">
...
$(document).ready(function(){
    $('.js-switch').change(function () {
        ...
        let story_id = $(this).data('id');
        var token = "{{csrf_token()}}";
        $.ajax({
            type: "POST",
            dataType: "json",
            url: '{{ route('admin.story.story_status_update') }}',
            data: {'status': status, 'story_id': story_id, _token:token},
