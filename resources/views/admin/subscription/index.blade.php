      ...
      <div class="content-header row">
         <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
               <h5 class="content-header-title float-left pr-1 mb-0">@lang('messages.subscription.subscription_users')</h5>
               <div class="breadcrumb-wrapper d-none d-sm-block">
                  <ol class="breadcrumb p-0 mb-0 pl-1">
                     <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="bx bx-home-alt"></i></a>
                     </li>
                     <li class="breadcrumb-item active">@lang('messages.subscription.subscription_users_list')
                     </li>
                  </ol>
               </div>
            </div>
         ...
         <div class="row">
            <div class="col-12">
               <div class="form-group">
                  <div class="col p-0">
                     <a href="{{route('admin.subscription.create')}}" class="btn btn-light-primary btn-sm" data-repeater-create="" type="button">
                     <i class="bx bx-plus"></i>
                      <span class="invoice-repeat-btn">Create New Message For Email</span>
                     </a>
                  </div>
               </div>
            </div>
                  ...
                  <div class="card">
                     ...
                     <div class="card-body card-dashboard">
                        <div class="table-responsive">
                            @if (App::isLocale('en'))
                               <table class="table zero-configuration" id="subscription_english_table">
                            @else
                               <table class="table zero-configuration" id="subscription_arabic_table">
                            @endif
                              <thead>
                                 <tr>
                                    <th>@lang('messages.common.sr_no')</th>
                                    <th>@lang('messages.common.email')</th>
                              ...
                              <tr>
                                 ...
                                 <td>
                                    @if (!empty($value['email_send_status'] == 'Success'))
                                          <span style="color:green">{{$value['email_send_status']}}</span>
                                    @else
                                          <span style="color:red">{{$value['email_send_status']}}</span>
                                    @endif
                              ...
                              @else
                              <tr>
                                 <td colspan="10">Subscription Users Not Found</td>
                              </tr>
                              @endif
                           </table>
                        </div>
...
$(document).ready(function(){
    $('.js-switch').change(function () {
        ...
        let subscription_user_id = $(this).data('id');
        var token = "{{csrf_token()}}";
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('admin.subscription.subscription_status_update') }}",
            data: {'status': status, 'subscription_user_id': subscription_user_id, _token:token},
